...
class BertModel(MegatronModule):
    ...
    def forward(self, bert_model_input, attention_mask, tokentype_ids=None, lm_labels=None):
        ...
        if args.profile:
            timers(f'bert_extended_attention_mask').start()
            
        extended_attention_mask = bert_extended_attention_mask(attention_mask)

        if args.profile:
            timers(f'bert_extended_attention_mask').stop()

        input_ids = bert_model_input

        if args.profile:
            timers(f'bert_position_ids').start()

        position_ids = bert_position_ids(input_ids)
        
        if args.profile:
            timers(f'bert_position_ids').stop()
        ...
        if self.post_process:
            if args.profile:
                timers(f'post_language_model_processing').start()

            result = post_language_model_processing(lm_output, pooled_output,
                                                  self.lm_head, self.binary_head,
                                                  lm_labels,
                                                  self.word_embeddings_weight(),
                                                  self.fp16_lm_cross_entropy)
            if args.profile:
                timers(f'post_language_model_processing').stop()
            return result
        else:
            return lm_output
